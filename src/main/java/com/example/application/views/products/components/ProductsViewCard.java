...
public class ProductsViewCard extends ListItem {
	...
	public ProductsViewCard(Product product, Category category, String url) {
		...
		imageDiv.addClassNames("background-div", "flex items-center", "justify-center", "mb-m",
				"overflow-product-view-card", "rounded-top w-full");
		...
		activeSpan.getElement().setAttribute("theme", product.getActiveStatus() ? "badge" : "badge error");
		activeSpan.setText(product.getActiveStatus() ? "Active" : "Inactive" );

		iceCreamDescriptionContainer.add(categorySpan, activeSpan);
