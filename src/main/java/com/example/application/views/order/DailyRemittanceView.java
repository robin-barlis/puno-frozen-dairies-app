...
@PageTitle("Daily Remittances")
@Route(value = "remittance", layout = MainLayout.class)
@RouteAlias(value = "/remittance", layout = MainLayout.class)
@RolesAllowed({ "Admin", "Superuser", "ADMIN" })
@Uses(Icon.class)
public class DailyRemittanceView extends AbstractPfdiView implements BeforeEnterObserver {
	...
	@Autowired
	public DailyRemittanceView(CustomerTagService customerTagService, CustomerService customerService,
...
//	
	...
	private void createProfileDialog(String label) {
		...
		saveButton.addClickListener(e -> {
			try {
				...
				UI.getCurrent().navigate(DailyRemittanceView.class);
	...
	public void beforeEnter(BeforeEnterEvent event) {
		...
		if (!customers.isEmpty()) {
			...
			refreshGrid();
			event.forwardTo(DailyRemittanceView.class);
		}
	}
