...
@Service
public class OrderSummaryReport2 {
	...
	public byte[] buildReport(Set<Order> orders, List<Size> sizes) throws ColumnBuilderException, ClassNotFoundException, JRException {
		
		
		List<JasperPrint> jasperPrints = Lists.newArrayList();
		List<ExporterInputItem> exporterInputItems = Lists.newArrayList();
		...
		for (Order order : orders) {
			ArrayList<HashMap<String, String>> rowsDataList = Lists.newArrayList();
			...
			JasperPrint jp = getReport(order, rowsDataList);
			jasperPrints.add(jp);
			
			SimpleExporterInputItem exportInputItem = new SimpleExporterInputItem(jp);
			exporterInputItems.add(exportInputItem);
		}
		...
		SimpleExporterInput exportInput = new SimpleExporterInput(exporterInputItems);
	...
	public JasperPrint getReport(Order order, ArrayList<HashMap<String, String>> rowsDataList)
		...
		DynamicReport dynaReport = getReport(order, headerStyle, detailTextStyle, detailNumberStyle);
	...
	private DynamicReport getReport(Order order, Style headerStyle, Style detailTextStyle, Style detailNumStyle)
